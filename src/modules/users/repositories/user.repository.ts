...
import { pool } from "/Users/na-gyeong/Desktop/Korea Aerospace University/UMC/umc-10th-nodejs-study/src/db.config.js";
...
export const addUser = async (data: any): Promise<number | null> => {
    ...
    const [result] = await pool.query<ResultSetHeader>(
        ...
        [
            ...
            data.detailAddress,
            data.phoneNumber,
            data.password, // ← bcrypt로 해시된 비밀번호를 저장합니다.
        ]
    );
...
// 5. 사용자가 좋아하는 음식 추가
export const addUserFood = async (userId: number, foodId: number): Promise<void> => {
    const conn = await pool.getConnection();

    try {
    await pool.query(
        `INSERT INTO user_food (user_id, food_id2) VALUES (?, ?);`,
        [userId, foodId]
    );
    } catch (err) {
    throw new Error(`오류가 발생했어요: ${err}`);
